...
n_feats <- nrow(combined)
matrix_size <- n_cells * n_feats
print(paste("Number cells:", n_cells, "; number features", n_feats))
...
if (matrix_size < 2^31 - 1) {
  ...
  print("Feature matrix less than 2^31 -1; using Matrix...")
  ...
  print("Feature matrix greater than 2^31 -1; chunking...")
  seurat_chunks <- chunk_seurat_object(se_base)

  bpc_counts <- c()
  for (i in seq_along(seurat_chunks)) {

    counts <- seurat_chunks[[1]]@assays[[1]]$counts

    path <- paste0("/root/Snuderl_allv7/chunk_", i, "_BP")
    BPCells::write_matrix_dir(
      mat = counts,
      dir = path,
      overwrite = TRUE
    )
    bpc_counts[[i]] <- BPCells::open_matrix_dir(dir = path)
  }
}

print("Chunking worked??")


# Extract and reorder metadata --
