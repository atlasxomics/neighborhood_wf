...
#' using enrichKEGG() with default settings applied (BH correction,
#' p-value < 0.05)
#'
#'
#' L. Franzén, 2021 June
...
d <- merge(d, eg, by = "gene_name", all = TRUE)
...
color_low <- "#441153"
color_low2 <- "#62376e"
p1 <- dotplot(pw, showCategory = 10) +
  ggtitle("GO") +
  scale_color_gradient(low = color_low2, high = "grey80")

p2 <- dotplot(pw_ra, showCategory = 10) +
  ggtitle("Reactome") +
  scale_color_gradient(low = color_low2, high = "grey80")

p3 <- dotplot(pw_kegg, showCategory = 10) +
  ggtitle("KEGG") +
  scale_color_gradient(low = color_low2, high = "grey80")
